...
export default function SignUpScreen({ navigation }) {
  ...
  const onSignUpPress = async () => {
    ...
    // Phone number validation removed since no longer required in Clerk dashboard

    setIsLoading(true);
    try {
      // Phone number no longer required for signup

      // Log the data being sent for debugging
      console.log('Creating signup with:', { emailAddress, username });

      // Create the signup with required fields (phone number disabled in Clerk dashboard)
      const signUpResult = await signUp.create({
        emailAddress: emailAddress,
        password,
        username,
        // Phone number removed since disabled in Clerk dashboard
      });
